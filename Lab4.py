...
class HashTable:
    ...
    def insert(self, word):
        item = self.create_number_word(word)
    ...
    def create_number_word(self,strn):
        count = 0    # Will keep track of the sum so far to calculate base-26 number
        length = len(strn)  #Number of letters in a word
        for letter in strn:
    ...
    def get_avrg(self): #Method to get average number of comparisons to retrieve element in HT
        average_row=0
        for i in range(self.initial_capacity):
    ...
    def search(self, word):
        key = self.create_number_word(word)
...
def number_words_list(file): # Method to know how many words are there in the file
    f1= open(file)
